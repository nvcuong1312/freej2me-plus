                                     ...
                                     M3Gbitmask *outSurfaceBits)
{
    ...
    if (!config || !eglGetConfigAttrib(g_display,
                                       config,
                                       EGL_SURFACE_TYPE,
                                         ...
                                         M3GNativeWindow wnd)
{
    ...
    EGLint redSize, greenSize, blueSize, alphaSize, surfacet, depth, rendert;
    eglGetConfigAttrib(display, config, EGL_RED_SIZE, &redSize);
    eglGetConfigAttrib(display, config, EGL_GREEN_SIZE, &greenSize);
    eglGetConfigAttrib(display, config, EGL_BLUE_SIZE, &blueSize);
    eglGetConfigAttrib(display, config, EGL_ALPHA_SIZE, &alphaSize);
    eglGetConfigAttrib(display, config, EGL_SURFACE_TYPE, &surfacet);
    eglGetConfigAttrib(display, config, EGL_RENDERABLE_TYPE, &rendert);
    eglGetConfigAttrib(display, config, EGL_DEPTH_SIZE, &depth);

    printf("[native] Window Surface EGLConfig attributes:\n");
    printf("  Red size: %d\n", redSize);
    printf("  Green size: %d\n", greenSize);
    printf("  Blue size: %d\n", blueSize);
    printf("  Alpha size: %d\n", alphaSize);
    printf("  surface type: %x\n", surfacet);
    printf("  depth size: %d\n", depth);
    printf("  render type: %x\n", rendert);
    ...
    surf = eglCreateWindowSurface(g_display,
                                  config,
                                         ...
                                         M3GNativeBitmap bmp)
{
    ...
    printf("[native] Pixmap Surface EGLConfig attributes:\n");
    printf("  Red size: %d\n", redSize);
    printf("  Green size: %d\n", greenSize);
    printf("  Blue size: %d\n", blueSize);
    printf("  Alpha size: %d\n", alphaSize);
    ...
    surf = eglCreatePixmapSurface(g_display,
                                  config,
                                          ...
                                          M3Gint width, M3Gint height)
{
    ...
    attrib[2] = EGL_HEIGHT;
    attrib[3] = height;
	/* attrib[4] = EGL_RENDER_BUFFER;
	attrib[5] = EGL_BACK_BUFFER; */
    attrib[4] = EGL_NONE;
	...
	if (!config) {
	printf("[native] m3gCreatePBufferSurface ERROR\n");
        return NULL;
    }
    ...
    printf("[native] Pbuffer Surface EGLConfig attributes:\n");
    printf("  Red size: %d\n", redSize);
    printf("  Green size: %d\n", greenSize);
    printf("  Blue size: %d\n", blueSize);
    printf("  Alpha size: %d\n", alphaSize);
	...
	EGLDisplay dpy = g_display;
	EGLint configID;
	eglQuerySurface(dpy,
					surf,
					EGL_CONFIG_ID,
					&configID);
	printf("[native] surf configID:%d\n",configID);
...
{
    EGLBoolean success = eglSwapBuffers(g_display,
                                        surface);
...
static void m3gUpdateTargetBuffer(RenderContext *ctx)
{
    if (ctx->target.type == SURFACE_IMAGE) {
        ...
        if (ctx->target.type == SURFACE_BITMAP) {
            ...
            if (fullClip && eglCopyBuffers(g_display,
                                     ...
                                     EGLSurface surface)
{
    ...
    /* No match found, we must create a new context */
    {
        ...
        //if (surfaceTypeBits == SURFACE_EGL)
        {
			...
			printf("[native] sdl2 eglGetConfigs num: %d\n", numConfigs);

			cfgs = malloc(numConfigs * sizeof(EGLConfig));
			if(cfgs == NULL) {
				return NULL;
			}
			...
			for(i = 0; i < numConfigs; i++) {
				...
				globalconf = cfgs[i];
				printf("[native] sdl2 set config: %d DEPTH_SIZE:%d\n", i, val);
				break;
			}
			free(cfgs); */
            ...
            glrc = eglCreateContext(dpy, m3gEGLConfigForConfigID(dpy, configID), shareRc/* EGL_NO_CONTEXT */,
									(EGLint *)egl_ctx_attr);
            //M3G_ASSERT(glrc);
        }
        /*else {
        ...
        */
		...
		printf("[native] eglCreateContext error=============\n");
			...
			EGLint err=eglGetError();
			printf("[native] eglGetErrorCode %d=============\n",err);
        ...
        {
            M3Gbool ok = eglMakeCurrent(g_display,
